...
def train_weights(gt_dist, gt_labels, num_steps, learning_rate):
    ...
    features = gt_labels
    # intercept = np.ones(shape=gt_dist.shape)
...
'''
# label=1 for same, label=0 for different
    ...
    for idx in range(len(lines)):
        ...
        for img in os.listdir(dir_train):
            ...
            if img == str(first + ".pgm"):
                # print("Finding second image...")
                for img_alt in os.listdir(dir_train):
                    if img_alt == str(second + ".pgm"):
                        path = dir_train + img
                        path_alt = dir_train + img_alt
                        vt_0 = extract_feature_vector(path)
                        # print("Extracting feature vector B...")
                        vt_1 = extract_feature_vector(path_alt)
                        # print("Finding Mahalanobis distance...")
                        # we use, instead of inverse, the Moore-Penrose inverse, in case encounter singular matrices.
                        # should be OK for calculating Mahalanobis matrix.
                        # per https://stats.stackexchange.com/questions/37743/singular-covariance-matrix-in-mahalanobis-distance-in-matlab
                        # print("shape of vt_0 is " + str(vt_0.shape))
                        # print("shape of vt_1 is " + str(vt_1.shape))

                        # TODO : clarify covariance for Mahalanobis distance calculation
                        # TODO : the Mahalabonis distance is derived statistically and needs to learn the covariance matrix from a set of datapoints
